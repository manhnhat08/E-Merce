...
public interface ProductRepository extends PagingAndSortingRepository<Product, Integer> {
	...
	@Query(nativeQuery = true,
            value = "SELECT * FROM ecommerce.products WHERE enabled= true ORDER BY id desc LIMIT 9")
	public List<Product> findAll();
	
	@Query(nativeQuery = true,
            value = "SELECT * FROM ecommerce.products WHERE enabled= true AND discount_percent >= 50 ORDER BY id desc LIMIT 5")
	public List<Product> findProductBestSeller();
	
	@Query(nativeQuery = true,
            value = "SELECT * FROM ecommerce.products WHERE enabled= true AND discount_percent = 25 ORDER BY id desc LIMIT 5")
